...
public class HanoiController {
    ...
    public HanoiController(HanoiModel model, HanoiView view, Stage stage) {
        ...
        this.stage = stage;
        bindHandlers();
        startNewGame(view.getDiscCountCombo().getValue());
    }

    // 1) Unification "reset" y "initialize" en un solo methods
    private void startNewGame(int discs) {
        if (solving) return;
    ...
    // 2) Centralizes binding de botones
    private void bindHandlers() {
        view.getDiscCountCombo().setOnAction(_ -> startNewGame(view.getDiscCountCombo().getValue()));
        view.getSolveButton().   setOnAction(_ -> beginSolve());
        view.getCancelButton().  setOnAction(_ -> stopSolve());
        view.getResetButton().   setOnAction(_ -> startNewGame(view.getDiscCountCombo().getValue()));
        view.getSaveButton().    setOnAction(_ -> saveMoves());
    }
